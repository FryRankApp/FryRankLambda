...
public class GetAllReviewsHandlerTests {
    ...
    public void testHandleRequest_WithZeroLimitParam_ClampsToOne() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(1), isNull(), eq(new ReviewFilter(null))))
                .thenReturn(defaultOutput);

        final APIGatewayV2HTTPResponse response = handler.handleRequest(createEvent(TEST_RESTAURANT_ID, null, "0", null), context);

        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(1), isNull(), eq(new ReviewFilter(null)));
    }
    ...
    public void testHandleRequest_WithLimitAboveMax_ClampsToMaxPageLimit() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(MAX_PAGE_LIMIT), isNull(), eq(new ReviewFilter(null))))
                .thenReturn(defaultOutput);

        final APIGatewayV2HTTPResponse response = handler.handleRequest(createEvent(TEST_RESTAURANT_ID, null, "999", null), context);

        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(MAX_PAGE_LIMIT), isNull(), eq(new ReviewFilter(null)));
    }

    @Test
    public void testHandleRequest_WithValidLimitParam_UsesProvidedLimit() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(25), isNull(), eq(new ReviewFilter(null))))
                .thenReturn(defaultOutput);

        final APIGatewayV2HTTPResponse response = handler.handleRequest(createEvent(TEST_RESTAURANT_ID, null, "25", null), context);

        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(25), isNull(), eq(new ReviewFilter(null)));
    }
    ...
    public void testHandleRequest_WithAccountIdAndValidLimit_PassesLimitToReviewDomain() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(isNull(), eq(TEST_ACCOUNT_ID), eq(5), isNull(), eq(new ReviewFilter(null))))
                .thenReturn(defaultOutput);

        final APIGatewayV2HTTPResponse response = handler.handleRequest(createEvent(null, TEST_ACCOUNT_ID, "5", null), context);

        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(isNull(), eq(TEST_ACCOUNT_ID), eq(5), isNull(), eq(new ReviewFilter(null)));
    }

    @Test
    public void testHandleRequest_WithRestaurantIdAndTag_ForwardsTagInReviewFilter() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(DEFAULT_PAGE_LIMIT), isNull(), eq(new ReviewFilter(TEST_TAG_1))))
                .thenReturn(defaultOutput);
        ...
        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(eq(TEST_RESTAURANT_ID), isNull(), eq(DEFAULT_PAGE_LIMIT), isNull(), eq(new ReviewFilter(TEST_TAG_1)));
    }

    @Test
    public void testHandleRequest_WithAccountIdAndTag_ForwardsTagInReviewFilter() throws Exception {
        doNothing().when(requestValidator).validateRequest(any(), any());
        when(reviewDomain.getAllReviews(isNull(), eq(TEST_ACCOUNT_ID), eq(DEFAULT_PAGE_LIMIT), isNull(), eq(new ReviewFilter(TEST_TAG_1))))
                .thenReturn(defaultOutput);

        final APIGatewayV2HTTPResponse response = handler.handleRequest(createEvent(null, TEST_ACCOUNT_ID, null, TEST_TAG_1), context);

        assertEquals(200, response.getStatusCode());
        verify(reviewDomain).getAllReviews(isNull(), eq(TEST_ACCOUNT_ID), eq(DEFAULT_PAGE_LIMIT), isNull(), eq(new ReviewFilter(TEST_TAG_1)));
    }
